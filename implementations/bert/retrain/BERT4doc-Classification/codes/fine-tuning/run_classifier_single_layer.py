...
class IMDBProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir, data_num=None):
        return self._create_test_examples(data_dir)
...
#         return examples
    ...
    def _create_examples(self, data_dir):
          ...
          data = data.dropna()
          data, _ = train_test_split(data, test_size=0.1, random_state=42)
          #data = data.head(10000)

          for i, row in data.iterrows():
              guid = "%s-%s" % ("train", i)
              examples.append(InputExample(guid=guid, text_a = tokenization.convert_to_unicode(row['Sentence']), text_b=None, label=row['Label']))
          return examples
    ...
    def _create_test_examples(self, data_dir):
        examples = []

        data = pd.read_csv(data_dir, header=None, index_col=0)
        data.columns = ["Sentence"]
        data.at[6124,'Sentence']='love' # TODO: change this!

        for i, row in data.iterrows():
            guid = "%s-%s" % ("dev", i)
            examples.append(InputExample(guid=guid, text_a=tokenization.convert_to_unicode(row['Sentence']), text_b=None, label=0))
...
def main():
    ...
    if args.do_predict:
        test_examples = processor.get_test_examples(args.test_data_dir, data_num=args.num_test_datas)
        test_features = convert_examples_to_features(
            test_examples, label_list, args.max_seq_length, tokenizer, trunc_medium=args.trunc_medium)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            with open(os.path.join(args.output_dir, "results_ep"+str(epoch)+".txt"),"w") as f, open(os.path.join(args.output_dir, "results_ep"+str(epoch)+"_X_predictions.csv"),"w") as f_X_predictions:
                f_X_predictions.write(',X\n')
                idx = 0
                for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluate"):
                    ...
                    for logit in logits:
                        res = np.exp(logit[0]) + np.exp(logit[1])
                        l = [np.exp(logit[0])/res, np.exp(logit[1])/res]
                        f_X_predictions.write(str(idx) + ',' + str(l) + "\n")
                        idx += 1
            ...
            # predict
            i=1
            with open(os.path.join(args.output_dir, "test_results_ep"+str(epoch)+".txt"),"w") as f, open(os.path.join(args.output_dir, "test_results_ep"+str(epoch)+"_X_test_predictions.csv"),"w") as f_X_test_predictions:
                f_X_test_predictions.write(',X\n')
                idx = 0
                for input_ids, input_mask, segment_ids, label_ids in tqdm(test_dataloader, desc="Testing"):
                    ...
                    for logit in logits:
                        res = np.exp(logit[0]) + np.exp(logit[1])
                        l = [np.exp(logit[0])/res, np.exp(logit[1])/res]
                        f_X_test_predictions.write(str(idx) + ',' + str(l) + "\n")
                        idx += 1
