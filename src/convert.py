...
with open(output_path+"train_data.txt", "w") as fw:

    with open(input_path+"train_pos.txt", "r") as fr1:
        line = fr1.readline()
        while line:
    ...
    with open(input_path+"train_neg.txt", "r") as fr2:
        line = fr2.readline()
        while line:
...
with open(output_path+"test_data.txt", "w") as fw:

    with open(input_path+"test_data.txt", "r") as fr:
        line = fr.readline()
        while line:
            tokens=line.split(',')
            new_line = tokens[0] + "," + '"'
