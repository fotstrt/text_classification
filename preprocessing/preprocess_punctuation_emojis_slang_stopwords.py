...
def preprocess_word(word):
    ...
    if word in ['<happyface>', '<veryhappyface>', '<tongueface>', '<heart>', '<wink>', '<sadface>', '<cryface>']:
        return word
...
def preprocess_tweet(tweet, slang_dict):
    ...
    for word in words:
        ...
        if word in contraction_dict:
            ...
            if (new_word != '' and is_valid_word(new_word)):
                processed_tweet.append(new_word)
     
    processed_tweet_new = []
    for word in processed_tweet:
        if (word not in our_stopwords):
            processed_tweet_new.append(word)

    tweet = ' '.join(processed_tweet_new)
...
if __name__ == '__main__':
    ...
    processed_file_name = sys.argv[1][:-4] + '-punctuation-emojis-slang-stopwords.csv'
